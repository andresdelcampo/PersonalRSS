...
- Browse confidence-aware **High**, **Maybe**, and **Filtered** views that follow you between feeds and survive page refreshes.
...
PersonalRSS starts with a transparent keyword baseline, then learns from the feedback you give. It considers recurring words and phrases in titles and summaries together with exact authors. Every article keeps its baseline, learned score, confidence evidence, manual override, and a readable explanation separate, so a vote can always be undone without losing the score it replaced.
...
The layout selector keeps the original **Reading** cards and adds two faster image-and-title browsing modes. **Scan** uses a large image on the left with the title on the right, while **Gallery** arranges image-led tiles across the available width. All layouts retain voting, the read-state control, and **Show full feed content**. The **Display** controls can hide voting or reveal the normally hidden **Learning** details—classification, automatic relevance, baseline, confidence, matching-feedback count, and explanations. Layout and display choices are remembered by the browser, and all three layouts collapse cleanly to a single column on narrow screens.
...
The preview's unread total deliberately counts every newly stored article across all three classifications. The source list shows separate unread High and Maybe badges, while each preview shows High, Maybe, and Filtered totals scoped to the selected Unread or All view. Four mutually exclusive feedback choices—**Very interested**, **Interested**, **Not interested**, and **Never this topic**—supply different positive or negative learning weights and immediately route the selected article to High or Filtered. Click the active choice again to undo it and restore automatic classification. The baseline, learned automatic score, confidence evidence, and manual effective score are stored separately, so feedback never destroys the values it replaced. A rated article stays in the current list even when its classification or read state changes, preserving context and the undo control; changing a top view or display control clears that temporary exception. Generated RSS items link back to their PersonalRSS preview because external readers cannot host interactive voting controls.

The local model learns from words and adjacent phrases in titles and summaries plus exact authors. It ignores common words, gives strong feedback twice the weight of ordinary feedback, caps its similarity adjustment, and explains the strongest matching evidence on every scored article. The logistic model uses the same explainable features with regularization. Five-fold cross-fitting keeps each rated story out of its own calibration prediction, and canonical URLs plus conservative title similarity prevent syndicated duplicates from overstating confidence. Refreshing a feed scores all fetched articles against one feedback snapshot, so the learning step does not make a database query for every article.
