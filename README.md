...
The layout selector keeps the original **Reading** cards and adds two faster image-and-title browsing modes. **Scan** uses a large image on the left with the title on the right, while **Gallery** arranges image-led tiles across the available width. Both compact modes retain the read-state control and **Show full feed content** action; expanding a post restores its complete metadata, content, scoring reason, and feedback controls in place. The selected layout is remembered by the browser and all three layouts collapse cleanly to a single column on narrow screens.
