...
The preview gives every post a prominent **Unread** or **Read** badge and a matching card treatment. That state uses the feed's explicit last-viewed marker: opening or expanding a post does not silently mark it read, while **Mark all read** updates every currently stored post. Each card also has an expandable **Show full feed content** section that renders the sanitized text, links, images, tables, and code supplied inside the RSS item without navigating or reloading; feeds that publish only an excerpt cannot expose content they did not include.
