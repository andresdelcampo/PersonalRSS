...
async function refreshFeed(feed){try{const response=await fetch(`/api/feeds/${feed.id}/refresh`,{method:'POST'});if(!response.ok)return{ok:false};const result=await response.json();newPostCounts.set(feed.id,result.newPosts);return{ok:true,newPosts:result.newPosts}}catch{return{ok:false}}}
...
async function loadAndRefresh(){let feeds=await getFeeds();renderFeeds(feeds);if(!feeds.length){message.textContent='';return}message.textContent=`Refreshing ${feeds.length} ${feeds.length===1?'feed':'feeds'}…`;const results=await refreshFeeds(feeds);feeds=await getFeeds();renderFeeds(feeds);const total=results.reduce((sum,result)=>sum+(result.newPosts??0),0),failed=results.filter(result=>!result.ok).length;message.textContent=`Refresh complete: ${total} new ${total===1?'post':'posts'}${failed?`; ${failed} ${failed===1?'feed':'feeds'} failed`:''}.`}
async function renameFeed(button){const name=prompt('Feed name',button.dataset.name);if(name===null||name.trim()===button.dataset.name)return;const response=await fetch(`/api/feeds/${button.dataset.id}`,{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({name})});message.textContent=response.ok?'Feed renamed.':await errorMessage(response,'Could not rename feed.');if(response.ok)renderFeeds(await getFeeds())}
...
const threshold=document.querySelector('#threshold'),scoreLabel=document.querySelector('#score-label');
let feed,articles=[];
async function start(){const feeds=await fetch('/api/feeds').then(r=>r.json());feed=feeds.find(f=>f.slug===slug);if(!feed){cards.innerHTML='<div class="empty">Feed not found.</div>';return}title.textContent=feed.name;document.title=`${feed.name} — PersonalRSS`;await load()}
...
function render(){const minimum=Number(threshold.value);scoreLabel.textContent=minimum.toFixed(1);cards.replaceChildren();const visible=articles.filter(a=>a.score>=minimum);status.textContent=`Showing ${visible.length} of ${articles.length} stored articles.`;if(!articles.length){cards.innerHTML='<div class="empty">No articles stored yet. Select “Refresh source” to fetch and score this feed.</div>';return}if(!visible.length){cards.innerHTML='<div class="empty">No articles meet this score threshold.</div>';return}for(const article of visible)cards.append(articleCard(article))}
function articleCard(article){const parsed=articleContent(article.summary,article.link);const card=document.createElement('article');card.className=`card${parsed.image?'':' no-image'}`;card.id=`article-${article.id.replaceAll('-','')}`;const body=document.createElement('div');const meta=document.createElement('div');meta.className='meta';meta.textContent=`${new Date(article.publishedAt).toLocaleString()} · relevance ${Number(article.score).toFixed(2)}`;const heading=document.createElement('h2');const link=document.createElement('a');link.href=article.link;link.target='_blank';link.rel='noopener noreferrer';link.textContent=article.title;heading.append(link);const summary=document.createElement('p');summary.className='summary';summary.textContent=parsed.text||'No summary supplied by this feed.';const reason=document.createElement('p');reason.className='reason';reason.textContent=article.scoreReason||'No scoring explanation available.';const actions=document.createElement('div');actions.className='actions';actions.append(voteButton(article,'Interested','More like this','secondary'),voteButton(article,'NotInterested','Less like this','negative'));body.append(meta,heading,summary,reason,actions);card.append(body);if(parsed.image){const image=document.createElement('img');image.className='image';image.src=parsed.image;image.alt='';image.loading='lazy';image.referrerPolicy='no-referrer';card.append(image)}return card}
function voteButton(article,kind,label,className){const button=document.createElement('button');button.className=className;button.textContent=label;button.onclick=async()=>{button.disabled=true;const response=await fetch(`/api/articles/${article.id}/feedback`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({kind})});if(response.ok){status.textContent=`Feedback saved for “${article.title}”.`;await load()}else{status.textContent='Could not save feedback.';button.disabled=false}};return button}
