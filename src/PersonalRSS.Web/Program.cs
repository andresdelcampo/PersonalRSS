...
<style>body{font:16px system-ui;max-width:900px;margin:3rem auto;padding:0 1rem;color:#17212b;background:#f5f7f9}main{background:white;padding:2rem;border-radius:14px;box-shadow:0 8px 30px #1b2a3812}h1{margin-top:0}h2{margin-bottom:.4rem}a{color:#126a5b}#feeds{list-style:none;padding:0}.source-item{margin:.65rem 0;padding:.8rem .9rem;border-left:4px solid #d3dce1;border-radius:8px;background:#f8fafb}.source-item.has-unread{border-left-color:#16866f;background:#edf8f4;box-shadow:0 2px 10px #126a5b0b}.feed-name{font-size:1.05rem;text-decoration-thickness:1px;text-underline-offset:2px}.has-unread .feed-name{color:#0b5b4c;font-weight:750}.is-read .feed-name{color:#52636d;font-weight:550}.unread-count{display:inline-block;margin-left:.4rem;padding:.15rem .48rem;border-radius:999px;font-size:.82rem}.has-unread .unread-count{background:#126a5b;color:white;font-weight:750;box-shadow:0 1px 4px #126a5b33}.is-read .unread-count{background:#e8edef;color:#687680;font-weight:500}form{display:grid;grid-template-columns:1fr 2fr auto;gap:.7rem;margin-top:.8rem}input,button{font:inherit;padding:.7rem;border:1px solid #bbc5cf;border-radius:8px}button{background:#126a5b;color:white;border:0;cursor:pointer}button.secondary{padding:.25rem .5rem;background:#e7eeec;color:#174d44}button:disabled{opacity:.6;cursor:wait}small{color:#607080}.message{min-height:1.5rem;margin:.25rem 0}.manage{margin-top:2rem;padding-top:1rem;border-top:1px solid #e1e7eb}.manage h2{font-size:1.1rem}details{margin:.65rem 0;border:1px solid #d8e0e5;border-radius:9px;background:#f8fafb}summary{padding:.7rem .9rem;color:#174d44;font-weight:600;cursor:pointer}details form{padding:0 .9rem .9rem;margin-top:.1rem}@media(max-width:650px){form{grid-template-columns:1fr}}</style></head>
...
async function load(){articles=await fetch(`/api/articles?feedId=${encodeURIComponent(feed.id)}&minScore=0&limit=200`).then(r=>r.json());render();await fetch(`/api/feeds/${feed.id}/viewed`,{method:'POST'})}
