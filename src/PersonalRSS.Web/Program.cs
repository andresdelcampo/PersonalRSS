...
function render(){const minimum=Number(threshold.value),unread=feed?.unreadCount??0;scoreLabel.textContent=minimum.toFixed(1);cards.replaceChildren();const visible=articles.filter(a=>a.score>=minimum||a.activeFeedback!=null);status.textContent=`Showing ${visible.length} of ${articles.length} stored articles. ${unread} unread across all scores.`;if(!articles.length){cards.innerHTML='<div class="empty">No articles stored yet. Select “Refresh source” to fetch and score this feed.</div>';return}if(!visible.length){cards.innerHTML='<div class="empty">No articles meet this score threshold.</div>';return}for(const article of visible)cards.append(articleCard(article))}
