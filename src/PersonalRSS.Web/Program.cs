...
function renderFeeds(feeds){list.replaceChildren();if(!feeds.length){list.innerHTML='<li>No feeds yet.</li>';return}for(const f of feeds){const li=document.createElement('li');const count=newPostCounts.get(f.id);li.innerHTML=`<strong>${esc(f.name)}</strong>${count===undefined?'':`<span class="new-count">${count} new ${count===1?'post':'posts'}</span>`} — <a href="/preview/${encodeURIComponent(f.slug)}">Preview</a> · <a href="/feeds/${encodeURIComponent(f.slug)}.xml">RSS</a> <button class="secondary rename" data-id="${f.id}" data-name="${esc(f.name)}">Rename</button><br><small>${esc(f.url)}${f.lastError?' · Error: '+esc(f.lastError):''}</small>`;list.append(li)}list.querySelectorAll('.rename').forEach(button=>button.onclick=()=>renameFeed(button))}
