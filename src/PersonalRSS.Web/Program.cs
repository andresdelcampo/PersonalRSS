...
function renderFeeds(feeds){list.replaceChildren();if(!feeds.length){list.innerHTML='<li>No feeds yet.</li>';return}for(const f of feeds){const li=document.createElement('li'),count=f.unreadCount??0;li.innerHTML=`<strong><a class="feed-name" href="/preview/${encodeURIComponent(f.slug)}">${esc(f.name)}</a></strong><span class="new-count">${count} unread</span> <button class="secondary rename" data-id="${f.id}" data-name="${esc(f.name)}">Rename</button><br><small>${esc(f.url)}${f.lastError?' · Error: '+esc(f.lastError):''}</small>`;list.append(li)}list.querySelectorAll('.rename').forEach(button=>button.onclick=()=>renameFeed(button))}
